$(window).on('load', function() {
  function e() {
    (document.querySelector('html').style.height = window.innerHeight + 'px'),
      ...
      (document.querySelector('#wrapper2').style.height =
        window.innerHeight + 'px'),
...
window.onload = function() {
  ...
  Star.prototype.update = function() {
	...
	//change mouse according to mouse movement
	this.lastMouse.x = (mouse.x-this.lastMouse.x-width/2)*0.005;
	this.lastMouse.y = (mouse.y-this.lastMouse.y-height/2)* 0.005;
	
	this.x+=(this.lastMouse.x);
	this.y+=(this.lastMouse.y);
    if (zoom) {
      let ABx = this.x - zoomPoint.x;
      let ABy = this.y - zoomPoint.y;

      //slope angle
      let angle = Math.atan2(ABy, ABx);

      //displacement in x direction
      var distance = Math.sqrt(ABx * ABx + ABy + ABy);
      this.x += zoomScale * 100 * Math.cos(angle);
      this.y += zoomScale * 100 * Math.sin(angle);
